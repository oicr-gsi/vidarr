...
`"maxEntitySize"` should be set to the maximum request size in MB that is allowed to be sent to
Víðarr

`"excludeWorkflowsFromProvenance"` determines which workflows, if any, should be excluded from the
data by the provenance endpoint.
...
This file does _not_ have to be on the Víðarr server and a copy of JARs and this script can be
placed on another server. They will also need a configuration file to be able to run workflows in
testing. It is a reduced form of the main configuration file:
