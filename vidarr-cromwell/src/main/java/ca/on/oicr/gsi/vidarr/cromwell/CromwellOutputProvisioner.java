          ...
          .sendAsync(
              HttpRequest.newBuilder()
                  .uri(
                      URI.create(
                          String.format(
                                  // Very large workflows can slow down cromwell when calls are requested. Limit how
          ...
          .thenAccept(
              s -> {
                ...
                switch (result.getStatus()) {
                  // In the case of failures ("Aborted" or "Failed"), request the full metadata from Cromwell
                  // so we can have call info for debugging.
                  case "Aborted":
                  case "Failed":
                    monitor.log(System.Logger.Level.INFO,
                            String.format("Cromwell job %s is failed, fetching call info on %s",
                    ...
                    CLIENT.sendAsync(
                            HttpRequest.newBuilder()
                                    .uri(
                                            URI.create(
                                                    String.format(
                                                            "%s/api/workflows/v1/%s/metadata", cromwellUrl, state.getCromwellId()
                                                    )
                                            )).timeout(Duration.ofMinutes(1)).GET().build(),
                            new JsonBodyHandler<>(MAPPER, WorkflowMetadataResponse.class)
                    ).thenApply(HttpResponse::body).thenAccept(
                            s2 -> {
                              final var fullResult = s.get();
                              monitor.log(System.Logger.Level.INFO, String.format("Successfully fetched full metadata for Cromwell job %s on %s", state.getCromwellId(), cromwellUrl));
                            ...
                            t2 -> {
                              ...
                              // TODO: this schedules 2 requests to cromwell /metadata now. Consider a failure-unique check
