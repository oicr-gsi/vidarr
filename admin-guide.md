...
It can be easiest to launch Víðarr with a shellscript as follows stored in `/srv/vidarr/run`:
    ...
    #!/bin/sh
    exec /usr/lib/jvm/java-21-openjdk-amd64/bin/java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:7000 --module-path $(ls /srv/vidarr/jars/*.jar|tr '\n' :) -m ca.on.oicr.gsi.vidarr.server/ca.on.oicr.gsi.vidarr.server.Main "$@"
...
`"maxEntitySize"` should be set to the maximum request size in MB that is allowed to be sent to Víðarr
...
For workflow developers, it is also useful to install a `/srv/vidarr/cli` script:
    ...
    #!/bin/sh
    exec /usr/lib/jvm/java-21-openjdk-amd64/bin/java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:7071 --module-path $(ls /srv/vidarr/jars/*.jar|tr '\n' :) -m ca.on.oicr.gsi.vidarr.cli/ca.on.oicr.gsi.vidarr.cli.Main "$@"
