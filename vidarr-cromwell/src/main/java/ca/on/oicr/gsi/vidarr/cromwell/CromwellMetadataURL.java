...
/**
 *  Heavily multithreaded Cromwell workflow runs have such a large 'calls' block that requesting it repeatedly
 *  can lock up Cromwell. Preserve Cromwell performance by limiting when we request that block to when we actually
 *  want to display that information.
 *  In production, that's only on Failed workflow runs. In development, we'd like to see it while processing is
 *  happening. The Cromwell WorkflowEngine and OutputProvisioner use configuration to control this behaviour.
 */
